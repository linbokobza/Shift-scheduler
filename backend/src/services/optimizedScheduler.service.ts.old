  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    for (const { day, shiftId } of shuffledShifts) {
      ...
      const candidates = employees.filter(emp => {
        // בדיקת זמינות בסיסית
        const avail = availabilityMap.get(emp.id);
        const { date } = allShifts.find(s => s.day === day && s.shiftId === shiftId) || { date: undefined };
        if (!isEmployeeAvailable(emp.id, day, shiftId, avail, vacationMap, date)) {
          return false;
        }
