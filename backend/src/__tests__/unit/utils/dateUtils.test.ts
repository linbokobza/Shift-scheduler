...
describe('dateUtils.service', () => {
  ...
  describe('isSubmissionDeadlinePassed', () => {
    ...
    it('is exactly true at the Tuesday 12:00:00 boundary itself', () => {
      // Week start Aug 2, 2026 -> deadline Tue Jul 21, 2026 12:00:00
      const weekStart = new Date(2026, 7, 2);

      jest.useFakeTimers();
      jest.setSystemTime(new Date(2026, 6, 21, 11, 59, 59)); // one second before
      expect(isSubmissionDeadlinePassed(weekStart)).toBe(false);

      jest.setSystemTime(new Date(2026, 6, 21, 12, 0, 0)); // exactly on the boundary
      expect(isSubmissionDeadlinePassed(weekStart)).toBe(true);
    });
  });
  ...
  describe('getSubmissionWeek', () => {
    ...
    it('Monday before the deadline: shows the current open week', () => {
      jest.useFakeTimers();
      jest.setSystemTime(new Date(2026, 6, 20, 9, 0, 0)); // Mon Jul 20, 2026 09:00
    ...
    it('Tuesday 11:59: still shows the current open week (not yet closed)', () => {
      jest.useFakeTimers();
      jest.setSystemTime(new Date(2026, 6, 21, 11, 59, 0)); // Tue Jul 21, 2026 11:59
    ...
    it('Tuesday 12:00:00 exactly: immediately switches to next week', () => {
      jest.useFakeTimers();
      jest.setSystemTime(new Date(2026, 6, 21, 12, 0, 0)); // Tue Jul 21, 2026 12:00:00
    ...
    it('Friday after the cutoff: shows the week that opened on Tuesday', () => {
      jest.useFakeTimers();
      jest.setSystemTime(new Date(2026, 6, 24, 9, 0, 0)); // Fri Jul 24, 2026 09:00
